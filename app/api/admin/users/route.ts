...
export async function GET(req: Request) {
  try {
    ...
    // Use APENAS service role (mais estável: sem RLS e sem depender do anon key)
    const supabaseAdmin = createClient(supabaseUrl, serviceRoleKey);

    // valida token e pega o caller
    ...
    if (roleErr) {
      return json(500, { ok: false, error: "admin_check_failed", details: roleErr.message });
    }
    ...
    while (true) {
      ...
      if (error) return json(500, { ok: false, error: "list_users_failed", details: error.message });
      ...
      if (rolesErr) return json(500, { ok: false, error: "roles_failed", details: rolesErr.message });
      ...
      for (const u of batch) {
        ...
        users.push({
          ...
          role: normalizeRole(roleItem?.role ?? "leitor"),
